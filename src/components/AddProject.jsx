    ...
    return (
        <div className='relative group p-6 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 shadow-2xl transition-all hover:bg-white/20 w-full'>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <input className='peer bg-transparent border-b-2 border-white/10 focus:border-blue-500 outline-none transition-all w-1/2' value={newProject.title} onChange={e => update('title',e.target.value)}/><br/> <br/>
                <label className='text-xl font-semibold text-white'>Description</label><br/>
                <input className=' peer bg-transparent border-b-2 border-white/10 focus:border-blue-500 outline-none transition-all w-1/2' value={newProject.description} onChange={e => update('description', e.target.value)}/> <br/><br/>
